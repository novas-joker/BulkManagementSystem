import { ArrowRight, BarChart3, Mail, Play, ShieldCheck, Sparkles, Users } from 'lucide-react'
...
function ArrowIcon() { return <ArrowRight className="arrow-icon" size={18} aria-hidden="true" /> }
function PlayIcon() { return <Play className="play-icon" size={16} aria-hidden="true" fill="currentColor" /> }
...
function FeatureIcon({ type }) {
  const icons = { people: Users, chart: BarChart3, spark: Sparkles, shield: ShieldCheck, paper: Mail }
  const Icon = icons[type] || ArrowRight
  return <span className={`feature-icon feature-icon--${type}`} aria-hidden="true"><Icon size={20} /></span>
}
